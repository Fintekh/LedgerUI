...
describe('DataTable Component', () => {
  ...
  test('rowClassName function is applied correctly', () => {
    const rowClassNameFn = (item: TestItem) => {
      return item.age > 30 ? 'senior-employee' : 'junior-employee';
    };
    ...
    expect(rows[0]).toHaveClass('junior-employee');
    
    // Jane Smith is 25, should have junior class
    expect(rows[1]).toHaveClass('junior-employee');
    
    // Bob Johnson is 40, should have senior class
    expect(rows[2]).toHaveClass('senior-employee');
  });
